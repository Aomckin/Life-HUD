...
import { ritualCopy as c } from "../content/copy.js?v=0.5.1";
...
function render(root, details) {
  ...
  const card = d => {
    ...
    return `<article class="card ritual-card"><div><span class="badge">${escapeHtml(r.category || "仪式")}</span>
      ...
      <small>${d.steps.length} 个步骤 · ${last?`上次完成 ${time(last.completedAt)}`:"还没有完成过"}</small></div>
      <div class="row-actions ritual-actions">
        ${r.enabled?`<button class="button button-primary" data-start="${r.id}">${c.start}</button>`:""}
  ...
  root.innerHTML = `<div class="direction-page"><section class="panel"><div class="section-head"><div><div class="eyebrow">v0.5 · Ritual</div><h2>进入一种状态</h2></div><button class="button button-primary" id="new-ritual">${c.newRitual}</button></div>
    ${enabled.map(card).join("")||empty(c.empty)}
  </section>
  ${disabled.length?`<section class="panel"><div class="section-head"><h2>${c.disabledTitle}</h2></div>${disabled.map(card).join("")}</section>`:""}
  <form class="milestone-form" id="ritual-form" hidden>
    ...
    <div><button class="button button-primary" type="submit">${c.save}</button><button class="button button-ghost" type="button" id="cancel-ritual">${c.cancel}</button></div>
  </form></div>`;
  ...
  root.querySelector("#new-ritual").addEventListener("click", () => { stepEditor.innerHTML = ""; addStepRow(); form.hidden = false; root.querySelector("#ritual-name").focus(); });
  root.querySelector("#cancel-ritual").addEventListener("click", () => form.hidden = true);
  ...
  form.onsubmit = async event => {
    ...
    try {
      await api.rituals.create({name: v("ritual-name"), description: v("ritual-description"), category: v("ritual-category"),
        triggerTime: v("ritual-time") || null, enabled: root.querySelector("#ritual-enabled").checked, steps});
      toast("仪式已创建"); rituals(root);
  ...
  async function openEdit(id) {
    ...
    form.onsubmit = async event => {
      ...
      try {
        await api.rituals.update(id, {name: root.querySelector("#ritual-name").value.trim(),
          description: root.querySelector("#ritual-description").value.trim(),
          category: root.querySelector("#ritual-category").value.trim(),
          triggerTime: root.querySelector("#ritual-time").value || null,
          enabled: root.querySelector("#ritual-enabled").checked, steps});
        toast("仪式已更新"); form.hidden = true; rituals(root);
      } catch (reason) { toast(reason.message, true); }
    };
  }
}
