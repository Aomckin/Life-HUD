...
public final class PlayerService {
    // addExp is the only EXP mutation primitive. Since v0.4.2 it may only be called by
    // GrowthEngine's SPEND settlement or an explicit admin/migration correction.
    ...
    /** @deprecated v0.4.2: tasks no longer grant rewards directly; they emit LifeEvents settled by GrowthEngine. */
    @Deprecated public void addDailyTaskReward(Player p,int energy,int exp){addEnergy(p,energy);addExp(p,exp);incrementTaskDone(p);} @Deprecated public void addSpecialTaskReward(Player p,int coin,int exp){addCoin(p,coin);addExp(p,exp);incrementSpecialTaskDone(p);}
    /** @deprecated v0.4.2: retired action path; its commands are rejected at GameCommandFacade. */
    @Deprecated public void addActionResult(Player p,String name,int energy,int exp){addEnergy(p,energy);addExp(p,exp);incrementAction(p,name);} @Deprecated public void completeAction(Player p,String name,int energy,int exp){addActionResult(p,name,energy,exp);}
    @Deprecated public Map<String,Object> completeTimedAction(Player p,String name,int minutes,int energy,int exp){addActionResult(p,name,energy,0);addExp(p,exp);incrementTimedAction(p);Map<String,Object> r=new LinkedHashMap<>();r.put("action_name",name);r.put("duration_minutes",minutes);r.put("energy_change",energy);r.put("exp_change",exp);r.put("current_energy",p.energy);return r;}
