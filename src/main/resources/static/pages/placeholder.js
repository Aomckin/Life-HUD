...
export async function placeholder(root, route) {
  ...
  root.innerHTML = `<section class="panel placeholder"><div class="eyebrow">Life HUD v0.2.2</div><h1>${labels[route]}</h1><p>这个入口已经接入 Summer Sky Shell。完整体验会在后续版本写进这片天空。</p>${empty("现在先把注意力留给今天。")}</section>`;
}
...
async function focus(root) {
  root.innerHTML = [
    ...
    '<div class="hero-copy"><div class="eyebrow">专注 · Focus</div><h1>从一件小事开始</h1><p>把注意力留给当下。选择一个方向，然后只处理眼前这一件事。</p><div class="focus-primary-row"><button class="button button-primary focus-primary" id="start-focus">开始专注</button><span class="focus-primary-note">进入专注准备；完整计时流程将在 v0.3 接入。</span></div><div class="focus-quick-group"><div class="focus-quick-label">Quick actions</div><div class="focus-actions" id="focus-actions">正在读取可用行动…</div></div></div>',
  ...
  root.querySelector("#start-focus").addEventListener("click", () => {
    toast("专注入口已准备好；完整 Focus Session 将在 v0.3 接入。");
  });
