...
> **当前正式版 v1.0.0「First Stable Release」**：Life HUD 是独立的本地单用户生活事实与业务系统，不包含 Agent Runtime。`Focus / Tasks / Dreams / Ritual / Life / Media / Journal / Now / Growth / Today` 共用 JSON 持久化、`LifeEvent`、Timeline 和 Growth；`/api/agent/context/*` 提供 `schemaVersion="1"` 的只读语义聚合，外部 Agent 写入复用 Business API，契约见 `AGENT_ACTION_API.md`。所有 API 错误统一为含 `status/detail/path/timestamp` 的 JSON；Today 内置 20 条外置文案，Dashboard 的 Dream / Ritual / Media 展示项可在对应页面自选并持久化，页面和 Dashboard 具备 reduced-motion 安全的轻量进入反馈。默认数据目录为 `./data`，可通过 `LIFEHUD_DATA_DIR` 配置，备份必须包含整个目录与 `uploads/`；所有图片、音频及内嵌封面按 SHA-256 内容去重，启动时会在重写 JSON 引用后安全归并历史副本。正式版自动化基线为 186 项；已通过隔离 data dir 全业务写入、停止/重启恢复、390/1366/1920 响应式、Console 0 error 及 `git diff --check` 验收。已知边界：无鉴权、无云同步、无内置 Agent、无第三方媒体同步，只适合本机或可信私网。后续只按真实需求演进，不为版本号新增大域。

> **当前分支 v0.8.1「Dashboard HUD 化」**：不改动 v0.8 Context API，仅重组 Today 的视觉层级。状态舱以每次打开随机抽取的「今日文案」为主标题，日期降级为普通信息，底部文案库支持增删；Check-in、Active Focus 与 Growth readout 汇入同一状态舱；四张今日事实卡收拢为一条指标带；Dream / Ritual / Media 共用一座陪伴舱但分别使用航向星标、日光节律和播放终端视觉语义；Timeline 改为轻透足迹轨道。宽屏内容扩至 10 列，所有主层使用低遮挡渐变、局部模糊和透光边界，使壁纸重新成为 Dashboard 的环境层。外部 Agent 的稳定对接契约维护在 `docs/AGENT_CONTEXT_API.md`。

> **当前分支 v0.8「Integration / 驾驶舱组装」（版本 0.8.0）**：新增 `LifeDateService` 统一本地日期边界；`AgentContextService` 作为 Dashboard 与 Agent Context 的共享只读聚合层，提供 `/today`、`/recent`、`/status`、`/focus`、`/tasks`、`/dreams`、`/life`、`/journal`、`/media`、`/growth` 十个强类型 API，统一包含 schemaVersion / generatedAt，并限制 days=1~30、journal limit=1~100。`GET /api/dashboard` 一次返回此刻、今日事实、方向陪伴与今日 Timeline；前端 Today 已改为单请求四层 Summer Sky 驾驶舱及 390px 响应式布局。Growth 新增统一 `AchievementEvaluator` 与外置跨域规则，覆盖 Dream、Ritual、Life、Focus 10/50/100 小时、Anime/Game、LifeEvent 总量及单日来源丰富度，Title 仍手动装备且不提供 Buff；既有轻量 Snapshot 已包含 Focus、Task、LifeEvent 聚合字段并保持旧存档兼容。完整自动化测试 172 项通过。版本未增加 Agent 写入、LLM、MCP 或外部集成。
