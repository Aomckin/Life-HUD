...
function render(root) {
  ...
  root.innerHTML = `<div class="timeline-page">
    <section class="panel timeline-intro"><div class="eyebrow">v0.6 · Journal</div>
      <h1>发生过的事,按时间留在这里。</h1>
    ...
    <div id="timeline-days">${days.size ? [...days.entries()].map(([key, list]) => `
      ...
      ${[...list].sort((a, b) => new Date(a.occurredAt) - new Date(b.occurredAt)).map(item => itemCard(item)).join("")}
      </section>`).join("")
      : empty("这条河暂时还是一张白纸。去 /life 记一笔,或写几句日记。")}
    </div>
...
function itemCard(item) {
  ...
  return `<article class="timeline-event ${item.source === "JOURNAL" ? "journal" : ""}" data-item="${item.eventId}">
    ...
    ${item.source === "JOURNAL" ? `<button class="text-link danger-link" data-remove-journal="${item.sourceId}">删除</button>` : ""}
  </article>`;
}
