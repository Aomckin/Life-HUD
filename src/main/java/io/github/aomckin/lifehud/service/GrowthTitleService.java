...
public final class GrowthTitleService {
    ...
    public GrowthTitleService(Player player,PlayerRepository players,JsonFileStore files,ObjectMapper mapper,LifeEventService events,GrowthCatalog catalog,GrowthCopy copy){this.player=player;this.players=players;this.files=files;this.mapper=mapper;this.events=events;this.catalog=catalog;this.copy=copy;}
    public List<Map<String,Object>> all(){List<GrowthCatalog.TitleDefinition> definitions=new ArrayList<>(catalog.titles());definitions.addAll(custom());Set<String> known=new HashSet<>();definitions.forEach(v->known.add(v.id()));if(files.exists("titles.json"))for(var node:files.read("titles.json")){String id=node.path("id").asText();if(player.unlocked_titles.contains(id)&&known.add(id))definitions.add(new GrowthCatalog.TitleDefinition(id,node.path("name").asText(id),node.path("desc").asText(copy.legacyTitleDescription()),"SYSTEM","migration",false,false));}for(String legacy:player.unlocked_titles)if(known.add(legacy))definitions.add(new GrowthCatalog.TitleDefinition(legacy,legacy,copy.legacyTitleDescription(),"SYSTEM","migration",false,false));return definitions.stream().map(this::view).toList();}
    ...
    public synchronized Map<String,Object> equip(String id){Map<String,Object> title=all().stream().filter(v->id.equals(v.get("id"))).findFirst().orElseThrow(()->bad("称号不存在"));if(!player.unlocked_titles.contains(id))throw bad("称号尚未解锁");player.equipped_title=id;players.save(player);events.record(LifeEventType.TITLE_EQUIPPED,"title",String.valueOf(title.get("name")),copy.titleEquippedDescription(),List.of("growth","title"),Map.of("titleId",id));return title;}
    public synchronized void unequip(){if(player.equipped_title.isBlank())return;String id=player.equipped_title;player.equipped_title="";players.save(player);events.record(LifeEventType.TITLE_EQUIPPED,"title",copy.titleUnequippedTitle(),copy.titleUnequippedDescription(),List.of("growth","title"),Map.of("titleId",id,"equipped",false));}
    ...
    public String currentName(){return all().stream().filter(v->Boolean.TRUE.equals(v.get("equipped"))).map(v->String.valueOf(v.get("name"))).findFirst().orElse(copy.noTitleSelected());}
