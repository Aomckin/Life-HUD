...
public final class DailyTaskManager {
    private static final int DAILY_DRAW_COUNT = 6;
    ...
    public void draw(){List<String> ids=unfinishedTaskIds();Collections.shuffle(ids,random);data.putArray("active_task_ids").addAll(ids.stream().limit(Math.min(DAILY_DRAW_COUNT,ids.size())).map(com.fasterxml.jackson.databind.node.TextNode::new).toList());}
    public boolean hasValidActiveTasks(){JsonNode ids=data.path("active_task_ids");List<String> unfinished=unfinishedTaskIds();if(!ids.isArray()||ids.size()!=Math.min(DAILY_DRAW_COUNT,unfinished.size()))return false;Set<String> valid=new HashSet<>(unfinished);Set<String> active=new HashSet<>();for(JsonNode id:ids)if(!valid.contains(id.asText())||!active.add(id.asText()))return false;return true;}
    ...
    public DailyTask addDefinition(String name,String note,int reward,int exp){
        ...
        DailyTask t=new DailyTask();t.id=id;t.name=name;t.note=note;t.reward=reward;t.exp=exp;
        t.createdTime=java.time.LocalDateTime.now();t.done=false;t.enabled=true;
        allTasks.add(t);save();draw();setActive();return t;
    }
    public DailyTask addDefinition(String name,int reward,int exp){return addDefinition(name,"",reward,exp);}
    public DailyTask updateDefinition(String id,String name,String note,int reward,int exp){
        DailyTask t=allTasks.stream().filter(v->v.id.equals(id)).findFirst().orElseThrow();
        t.name=name;t.note=note;t.reward=reward;t.exp=exp;save();setActive();return t;
    }
    public DailyTask updateDefinition(String id,String name,int reward,int exp){DailyTask t=allTasks.stream().filter(v->v.id.equals(id)).findFirst().orElseThrow();return updateDefinition(id,name,t.note,reward,exp);}
