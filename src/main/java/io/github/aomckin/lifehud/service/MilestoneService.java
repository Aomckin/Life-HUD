...
public final class MilestoneService {
    private final MilestoneRepository values;private final LifeEventService events;private final GrowthCopy copy;
    public MilestoneService(MilestoneRepository values,LifeEventService events,GrowthCopy copy){this.values=values;this.events=events;this.copy=copy;}
    public List<Milestone> all(){return values.all();}
    public synchronized Milestone create(MilestoneRequest request){validate(request);Instant now=Instant.now();Milestone value=new Milestone(UUID.randomUUID().toString(),request.title().trim(),clean(request.description()),request.occurredAt()==null?LocalDate.now():request.occurredAt(),defaulted(request.category(),copy.milestoneDefaultCategory()),MilestoneSource.MANUAL,request.relatedEventIds(),clean(request.relatedModule()),clean(request.media()),Boolean.TRUE.equals(request.pinned()),now,now);values.save(value);events.record(LifeEventType.MILESTONE_CREATED,"milestone",value.title(),value.description(),List.of("growth","milestone"),Map.of("milestoneId",value.id(),"occurredOn",value.occurredAt().toString()));return value;}
